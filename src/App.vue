<template>
  <div class="h-screen w-width bg-orange-400 flex flex-col justify-center items-center">
    <div class="header">        
        <h1 class="text-4xl">JB Todo App w/ Tailwind & VUE</h1>
        <div class="form flex">
            <input 
            v-model="newTodo"
            @keyup="checkAdd"
    ...
    <div class="todos w-1/2 mt-8">
        <div class="pending" v-if="totalPendings>0">
            <h3 class="text-2xl">Pending: {{totalPendings}} tasks</h3>
            <ul id="pendingList">
                <li
                v-for="(pending, index) in pendingList"
                :key="index"
                @click="moveToCompleted(pending, index)" 
                class="text-center p-3 text-red-500 bg-red-200 mt-4 rounded shadow-lg cursor-pointer hover:bg-red-700 hover:text-white"
                >{{pending}}</li>
            </ul>
        </div>
        <div class="completed mt-8" v-if="totalCompleted>0">
            <h3 class="text-2xl">Completed: {{totalCompleted}} tasks</h3>
            <ul id="completedList">
                <li 
                v-for="(completed, index) in completedList"
                :key="index" 
                @click="moveToPending(completed, index)"
                class="text-center p-3 line-through text-green-500 bg-green-200 mt-4 rounded shadow-lg cursor-pointer hover:bg-green-700 hover:text-white"
                >{{completed}}</li>
            </ul>
        </div>
    </div>
